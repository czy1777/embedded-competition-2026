...
static rt_err_t uart_common_rx_ind(rt_device_t dev, rt_size_t size)
{
    ...
    if (read_size > 0) {
        /* 将数据写入环形缓冲区 */
        rt_size_t put_size = rt_ringbuffer_put(&port->ringbuffer, temp_buffer, read_size);
        if (put_size < read_size) {
            /* 缓冲区满，部分数据丢失 - 可以通过 rt_kprintf 记录 */
        }
    }
...
static rt_err_t uart_port_init(uart_port_id_t port_id)
{
    ...
    if (port->device->open_flag != 0) {
        /* 设备已被打开，需要关闭后重新以中断模式打开 */
//        rt_kprintf("%s already opened, reopen with INT_RX mode.\n", port->name);
        ...
        if (result != RT_EOK) {
            rt_kprintf("Reopen %s failed!\n", port->name);
            return -RT_ERROR;
        }
    } else {
        /* 配置串口参数 */
        /* UART1: 115200 (控制台), UART2: 9600 (JDY-18 to SkyStar),
         * UART3: 9600 (GPS), UART4: 9600 (JDY-18 to SmartWatch) */
        switch (port_id) {
            case UART_PORT_1:
                config.baud_rate = BAUD_RATE_115200;
                break;
            case UART_PORT_2:
                config.baud_rate = BAUD_RATE_9600;   /* JDY-18 (SkyStar 链路) 默认 9600 */
                break;
            case UART_PORT_3:
                config.baud_rate = BAUD_RATE_9600;
                break;
            case UART_PORT_4:
                config.baud_rate = BAUD_RATE_9600;   /* JDY-18 (Smart_Watch 链路) 默认 9600 */
                break;
            default:
                config.baud_rate = BAUD_RATE_115200;
                break;
    ...
    /* 创建数据处理线程 */
    static const int port_nums[] = {1, 2, 3, 4};
...
//    rt_kprintf("%s initialized successfully.\n", port->name);
 ...
 * @brief 向 UART1 格式化输出
 *
 * UART1 是 finsh 控制台，不由应用层接管，这里直接转发到 rt_kprintf，
 * 调用方（如 atgm336h GPS 日志）保持原签名不变。
 */
